...
T05.1 changed derived metrics and the sweep only, never A/B/C. Geometry parameters were read from code: (a=E_0=1), (ka=0.1), (f_0=0), (f_1in{0.1,0.4,0.8,1.0}); chain ((-d,0),(0,0),(d,0)), equilateral ((-d/2,-\sqrt3d/6),(d/2,-\sqrt3d/6),(0,\sqrt3d/3)), and centered fixed-shape scalene. The sweep is 3 geometries × 4 contrasts × 160 distances (d_{\min}/ain[2.1,10]), hence 1,920 rows; regression has nine rows. It records 92/92 tests, force-oracle (sim2.72\times10^{-16}), (s_{10}sim6.97\times10^{-17}), forbidden mode (sim1.04\times10^{-20}), residual (4.58\times10^{-16}), condition (1.403), telescope identity and canonical regressions preserved. Central-chain angle is `NaN`; both angular maxima over 640 chain rows are (0^\circ); 428 contaminated B--C entries were corrected, RMS columns obey (F_{\rm new}=\sqrt2F_{\rm old}) with max deviation (4.44\times10^{-16}). Commands include `python -m pytest -q`, `python -m pytest -q -W error`, `git diff --check`, `git status --short`, `git diff --stat`, `git diff --name-only`, and `sha256sum ...`; artifact regeneration was performed separately in T05.1. Environment T05.1a: Python 3.12.3, NumPy 2.5.1, SciPy 1.18.0, Matplotlib 3.11.1. Same environment is byte-deterministic; other versions can change final bits or PNG bytes without method randomness. Limits: Rayleigh, (N=3), (L_{\max}=1), A/B/C only; no D, higher orders or (N>3); scalene nonzero sum is not automatically a total cluster force.

## T05 / T05.1 / T05.1a / T05.1b audit record
